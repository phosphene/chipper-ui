...
export function StageNav({ canAdvance, onAdvance, onBack, advanceLabel = 'Confirm & Continue →', showBack = true }: Props) {
  return (
    <div className="flex justify-between items-center mt-4 pt-3 border-t border-white/07">
      {showBack && onBack
        ? <button onClick={onBack} className="px-4 py-2 border border-white/10 rounded-md text-[#555] text-[0.78rem] font-mono hover:border-white/20 hover:text-[#888] transition-all">← Back</button>
        : <span />}
